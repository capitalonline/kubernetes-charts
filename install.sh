...
说明:
  本脚本是安装系统的入口脚本，负责：
  1. 同步代码仓库（git pull）
  2. 调用真正的安装脚本 scripts/install-components.sh
  
  这样设计的好处：
  - 即使 install.sh 本地文件较旧，也能拉取最新的安装脚本
  - 确保每次安装都使用最新版本的安装逻辑
...
注意:
  ...
  - 真正的安装脚本位于 ${CHARTS_REPO_DIR}/scripts/install-components.sh
  - 所有命令选项（-n, -v, -d, -l 等）会透传给安装脚本
...
function main() {
    ...
    if [[ "${ENABLE_LOG_FILE}" == "true" ]]; then
        ...
        # 写入日志头部
        echo "======================================" >> "${LOG_FILE}"
        echo "Kubernetes 组件安装日志" >> "${LOG_FILE}"
    ...
    # 调用真正的安装脚本
    log_step "调用安装脚本"
    log_info "执行: ${CHARTS_REPO_DIR}/scripts/install-components.sh ${all_args[*]}"
    
    # 如果启用了日志，传递日志文件路径给安装脚本
    if [[ "${ENABLE_LOG_FILE}" == "true" ]]; then
        log_info "日志将保存到: ${LOG_FILE}"
        exec "${CHARTS_REPO_DIR}/scripts/install-components.sh" --log-file "${LOG_FILE}" "${all_args[@]}"
    else
        exec "${CHARTS_REPO_DIR}/scripts/install-components.sh" "${all_args[@]}"
    fi
}
